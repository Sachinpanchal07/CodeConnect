...
const ViewConnection = () => {
  ...
  return (
    <>
        { loading ? <BeatLoader className="pt-50 text-center min-h-screen"></BeatLoader> :
            ...
            <div className="min-h-[400px] flex flex-col w-full sm:w-2xl bg-gray-950/50 mx-auto p-5 sm:p-8 sm:rounded-lg">
                ...
                <figure className="flex-shrink-0 ">
                    <img src={user?.photoUrl} alt="" className="bg-blue-500 h-35 w-35 rounded-full border-2 border-white" />
                </figure>
                <div className="">
                    <h2 className="text-2xl font-bold pt- flex items-center gap-2">
                    {user?.firstName + " " + user?.lastName}
                    {user?.isPremium  && <span className="flex items-center justify-center w-5 h-5 rounded-full bg-blue-500 text-white text-xs font-bold">
                        ✓
                    </span>}
                    </h2>
